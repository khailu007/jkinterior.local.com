...
/**
 * Add Design Shop - Post type

 */

function we_designshop_custom_post_type(){
    /*
     * Biến $label để chứa các text liên quan đến tên hiển thị của Post Type trong Admin
     */
    $label = array(
        'name' => 'Thiết kế Shop', //Tên post type dạng số nhiều
        'singular_name' => 'Thiết kế Shop', //Tên post type dạng số ít
    ...
    $args = array(
        ...
        'description' => 'Thiết kế Shop', //Mô tả của post type
        'supports' => array(
            'title',
        ...
        'capability_type' => 'post', //
 		'rewrite' => array('slug' => 'thiet-ke-shop'),
    );

 
    register_post_type('design_shop', $args); //Tạo post type với slug tên là sanpham và các tham số trong biến $args ở trên

    register_taxonomy('cat_design_shop', 'design_shop', array(
		'hierarchical' 			=> true,
      ...
      //   'menu_name' => 'Tags Desgin Shop',
      // ); 

      // register_taxonomy('tagdesignshop','design_shop', array(
      //   'hierarchical' => false,
      //   'labels' => $labels,
      //   'show_ui' => true,
      //   'update_count_callback' => '_update_post_term_count',
      //   'query_var' => true,
      //   'rewrite' => array( 'slug' => 'tagdesignshop' ),
      // ));
 
}
add_action('init', 'we_designshop_custom_post_type');

/**
 * Desin Shop Post custom meta fields.
 */


function pix_design_shop_meta_add(){

	global $pix_design_shop_meta_box;
	...
	$pix_design_shop_meta_box = array(
		'id' 		=> 'pix-meta-post',
		'title' 	=> __('PixFort Post Options','pix-opts'),
		'page' 		=> 'design_shop',
		'post_types'	=> array('design_shop'),
		'context' 	=> 'normal',
		'priority' 	=> 'default',
		'fields'	=> array(
	...
	add_meta_box($pix_design_shop_meta_box['id'], $pix_design_shop_meta_box['title'], 'pix_design_shop_show_box', $pix_design_shop_meta_box['page'], $pix_design_shop_meta_box['context'], $pix_design_shop_meta_box['priority']);
}

add_action('admin_menu', 'pix_design_shop_meta_add');

function pix_design_shop_show_box() {
	global $pix_design_shop_meta_box, $post;

	// Use nonce for verification
	echo '<div id="pix-wrapper">';
		echo '<input type="hidden" name="pix_design_shop_meta_nonce" value="', wp_create_nonce(basename(__FILE__)), '" />';

		echo '<table class="form-table">';
			echo '<tbody>';

				foreach ($pix_design_shop_meta_box['fields'] as $field) {
...
/*-----------------------------------------------------------------------------------*/
function pix_design_shop_save_data($post_id) {
	global $pix_design_shop_meta_box;

	// verify nonce
	if( key_exists( 'pix_design_shop_meta_nonce',$_POST ) ) {
		if ( ! wp_verify_nonce( $_POST['pix_design_shop_meta_nonce'], basename(__FILE__) ) ) {
			return $post_id;
		}
	}
	...
	if(!empty($pix_design_shop_meta_box)){
		foreach ( (array)$pix_design_shop_meta_box['fields'] as $field ) {
...
add_action('save_post', 'pix_design_shop_save_data');
...
/*	Styles & scripts
/*-----------------------------------------------------------------------------------*/
function pix_design_shop_admin_styles() {
...
add_action('admin_print_styles', 'pix_design_shop_admin_styles');

function pix_design_shop_admin_scripts() {
...
add_action('admin_print_scripts', 'pix_design_shop_admin_scripts');
